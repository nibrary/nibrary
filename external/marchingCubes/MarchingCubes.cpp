...
void MarchingCubes::print_cube() { NIBR::disp(MSG_DEBUG,"\t%f %f %f %f %f %f %f %f\n", _cube[0], _cube[1], _cube[2], _cube[3], _cube[4], _cube[5], _cube[6], _cube[7]) ; }
//_____________________________________________________________________________
...
*/
  ...
  NIBR::disp(MSG_DETAIL,"Marching Cubes ran in %lf secs.", (double)(clock() - time)/CLOCKS_PER_SEC) ;
}
//_____________________________________________________________________________
...
//-----------------------------------------------------------------------------
{
  ...
  default : NIBR::disp(MSG_DEBUG, "Invalid face code %d\n", face ) ;  print_cube() ;  A = B = C = D = 0 ;
  };
...
//-----------------------------------------------------------------------------
{
  ...
  case 13 :
    ...
    case 11 :
      ...
      break ;
    default : NIBR::disp(MSG_DEBUG, "Invalid edge %d\n", edge ) ;  print_cube() ;  break ;
    }
    break ;

  default : NIBR::disp(MSG_DEBUG, "Invalid ambiguous case %d\n", _case ) ;  print_cube() ;  break ;
  }
...
//-----------------------------------------------------------------------------
{
  ...
  case 13 :
    ...
    switch( subconfig13[_subconfig] )
    {
      ...
      default :
        NIBR::disp(MSG_DEBUG,"Marching Cubes: Impossible case 13?\n" ) ;  print_cube() ;
      }
      break ;
...
//-----------------------------------------------------------------------------
{
  ...
  for( int t = 0 ; t < 3*n ; t++ )
  {
    ...
    if( tv[t%3] == -1 )
    {
      NIBR::disp(MSG_DEBUG,"Marching Cubes: invalid triangle %d\n", _ntrigs+1) ;
      print_cube() ;
    }
    ...
    if( t%3 == 2 )
    {
      if( _ntrigs >= _Ntrigs )
      {
        ...
        delete[] temp ;
        NIBR::disp(MSG_DEBUG,"%d allocated triangles\n", _Ntrigs) ;
        _Ntrigs *= 2 ;
      }
...
void MarchingCubes::test_vertex_addition()
{
  if( _nverts >= _Nverts )
  {
    ...
    delete[] temp ;
    NIBR::disp(MSG_DEBUG,"%d allocated vertices\n", _Nverts) ;
    _Nverts *= 2 ;
  }
}
