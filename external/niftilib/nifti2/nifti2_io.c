...
static const char gni_version[] = " (18 Jun, 2020)";
...
/*! return whether the filename is valid
    ...
    ".nii"              |  0      | <MSG_ERROR - basename has zero length>
...
#undef  ERREX
#define ERREX(msg)                                                        \
 do{ fprintf(stderr,"** MSG_ERROR: nifti_convert_n1hdr2nim: %s\n", (msg) ) ;  \
     return NULL ; } while(0)
...
#undef  ERREX
#define ERREX(msg)                                           \
 do{ fprintf(stderr,"** MSG_ERROR: nifti_convert_n2hdr2nim: %s\n", (msg) ) ;  \
     return NULL ; } while(0)
...
#undef  ERREX
#define ERREX(msg)                                           \
 do{ fprintf(stderr,"** MSG_ERROR: nifti_image_open(%s): %s\n",  \
...
#undef  ERREX
#define ERREX(msg)                                           \
 do{ fprintf(stderr,"** MSG_ERROR: nifti_image_read(%s): %s\n",  \
             (hname != NULL) ? hname : "(null)" , (msg) ) ;  \
     return NULL ; } while(0)
*/
...
static znzFile nifti_image_load_prep( nifti_image *nim )
{
   ...
   {
      if ( g_opts.debug > 0 ){
         if( !nim ) fprintf(stderr,"** MSG_ERROR: N_image_load: no nifti image\n");
         else fprintf(stderr,"** MSG_ERROR: nifti_image_load: bad params (%p,%d,"
...
#undef  ERREX
#define ERREX(msg)                                               \
 do{ fprintf(stderr,"** MSG_ERROR: nifti_read_buffer: %s\n",(msg)) ;  \
     return 0; } while(0)
*/
                                ...
                                nifti_image *nim)
{
  ...
  if( dataptr == NULL ){
     if( g_opts.debug > 0 )
        fprintf(stderr,"** MSG_ERROR: nifti_read_buffer: NULL dataptr\n");
     return -1;
  }
...
int64_t nifti_write_buffer(znzFile fp, const void *buffer, int64_t numbytes)
{
   ...
   int64_t ss;
   if (znz_isnull(fp)){
      fprintf(stderr,"** MSG_ERROR: nifti_write_buffer: null file pointer\n");
      return 0;
   }
                         ...
                         const nifti_brick_list * NBL)
{
   ...
   if( !NBL ){ /* just write one buffer and get out of here */
      if( nim->data == NULL ){
         fprintf(stderr,"** NIFTI MSG_ERROR (NWAD): no image data to write\n");
         return -1;
      }
      ...
      if (ss < nim->nbyper * nim->nvox){
         fprintf(stderr,
            "** NIFTI MSG_ERROR (NWAD): wrote only %" PRId64 " of %" PRId64
            " bytes to file\n",
...
static int nifti_write_extensions(znzFile fp, nifti_image *nim)
{
   ...
   if( nifti_write_buffer(fp, extdr, 4) != 4 ){
      fprintf(stderr,"** NIFTI MSG_ERROR: failed to write extender\n");
      return -1;
   }
...
#undef  ERREX
#define ERREX(msg)                                                \
 do{ fprintf(stderr,"** MSG_ERROR: nifti_image_write_hdr_img: %s\n",(msg)) ;  \
     return fp ; } while(0)
   ...
   while( ipos < slen && !ISEND(str[ipos]) ){
     ...
     while( isspace((int) str[ipos]) ) ipos++ ;   /* skip blanks */
      ...
      if( str[ipos] == '$' ){  /* special case */
         ...
         if( ibot < 0 ){
           fprintf(stderr,"** NIFTI MSG_ERROR: list index %" PRId64
                   " is out of range 0..%" PRId64 "\n",
                   ibot,nvals-1) ;
      ...
      if( str[ipos] == '-' ){
         ...
         fprintf(stderr,"** NIFTI MSG_ERROR: index list syntax is bad: '%s'\n",
                 str+ipos) ;
         free(subv) ; return NULL ;
      }
      ...
      if( str[ipos] == '$' ){  /* special case */
         ...
         if( itop < 0 ){
           fprintf(stderr,"** NIFTI MSG_ERROR: index %" PRId64
                   " is out of range 0..%" PRId64 "\n",
                   itop,nvals-1) ;
           free(subv) ; return NULL ;
         }
      ...
      if( str[ipos] == '(' ){  /* decode an integer */
         ...
         if( istep == 0 ){
           fprintf(stderr,"** NIFTI MSG_ERROR: index loop step is 0!\n") ;
           free(subv) ; return NULL ;
         }
         nused = (cpt-(str+ipos)) ;
         ipos += nused ;
