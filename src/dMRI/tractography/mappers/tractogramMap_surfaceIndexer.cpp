...
void NIBR::index2surface(TractogramReader& tractogram, Surface& surf, float sigma, std::string filePrefix)
{
    ...
    disp(MSG_DETAIL,"Creating output files for surface indexing.");
    ...
    disp(MSG_DETAIL,"Reading surface mesh");
    surf.readMesh();

    disp(MSG_DETAIL,"Computing neigboring vertices");
    surf.getNeighboringVertices();
    
    disp(MSG_DETAIL,"Starting tractogram to surface mapping");
    ...
    disp(MSG_DETAIL,"Setting surface indexing parameters");
    ...
    disp(MSG_DETAIL,"Started batch processing");
    while (remaining > 0) {
...
std::unordered_map<int,std::unordered_map<int, float>> NIBR::readSurfaceIndexingForSelection(const std::vector<int>& streamlineIndices, const std::string& indexFilePrefix, const std::vector<std::streampos>& positions) {
