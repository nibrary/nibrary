...
std::tuple<bool,float> NIBR::Pathway::isSegmentExiting(const LineSegment& segment, int ruleNo) {
    ...
    switch (srcType[ruleNo]) {
        ...
        case sph_src: {
            ...
            float q2c[3];
            vec3sub(q2c, sphCenter[ruleNo], segment.end);
        ...
        case surf_src: {
            ...
            if (!isnan(distance) && towardsOutside) {
                crossDist = distance;
                crossDist = std::clamp(crossDist,0.0f,segment.len);
                disp(MSG_DEBUG, "Exited rule (in->out) %d at face %d with %.4f / %.4f at intersection.", ruleNo, intersectingFaceInd, float(distance), segment.len);
                return std::make_tuple(true,crossDist);
            }
