...
NIBR::Walker *NIBR::Pathway::tieRequireRules(float* p, NIBR::Walker *w)
{
	...
	if (w->side == side_A)
	{
		...
		for (std::vector<PathwayRule>::iterator it = prules.begin(); it != prules.end(); ++it)
		{
			...
			if ((it->side == side_A) && (it->type == req_end_inside))
			{
				...
				if (!isPointInsideRule(p,it->uniqueId) && !isPointAtEdgeOfRule(p,it->uniqueId,EPS2))
				// if (!isPointInsideRule(p,it->uniqueId))
				{
	...
	else if (w->side == side_B)
	{
		...
		{
			if ((prules[n].type == req_end_inside) && (isPointInsideRule(p,n) || isPointAtEdgeOfRule(p,n,EPS2)))
