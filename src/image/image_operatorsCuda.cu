...
__device__ void dCUDA_imgResample(NIBR::CUDAImage<float>* img, uint* ijk, float** M, NIBR::CUDAImage<float>* imgOut)
{
...
__global__ void gCUDA_imgResample(NIBR::CUDAImage<float>* img, float** M, NIBR::CUDAImage<float>* imgOut)
{
...
void NIBR::imgResample(NIBR::CUDAImage<float>* img, float** M, NIBR::CUDAImage<float>* imgOut)
{


    float** dPointerInDevice;
    float** dPointerInHost = (float**)malloc(3*sizeof(float*));
    cudaMalloc((void**)&dPointerInDevice,3*sizeof(float*));
    for (int i = 0; i < 3; i++) {
        cudaMalloc((void **)&(dPointerInHost[i]),4*sizeof(float));
        cudaMemcpy(dPointerInHost[i],M[i],4*sizeof(float),cudaMemcpyHostToDevice);
    }
    cudaMemcpy(dPointerInDevice,dPointerInHost,3*sizeof(float*),cudaMemcpyHostToDevice);
...
template<typename T1,typename T2, typename T_OUT>
void NIBR::imgResample(NIBR::CUDAImage<T1>* img, T2** M, NIBR::CUDAImage<T_OUT>* imgOut) 
{
